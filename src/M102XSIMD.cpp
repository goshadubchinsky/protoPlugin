...
struct M102XSIMD : Module {
    ...
    void process(const ProcessArgs& args) override
    {
		...
		for (int x = 0; x < channels; x += batch_size)
		{
			...
			input_param_batch[batch_index] = params[INPUT_PARAM].getValue();
			input_param_batch[batch_index] *= range_param;

			if (inputs[CV2_INPUT].isConnected())
			{
				if (inputs[CV2_INPUT].getChannels() > 1)
				{
					cv_2_batch[batch_index] = xsimd::load_unaligned(&cv_2_voltage_ptr[x]);
				}
				else
				{
					cv_2_batch[batch_index] = cv_2_voltage_ptr[0];
				}
				
				input_cv_param_2 = params[CV2_PARAM].getValue();
				cv_2_batch[batch_index] = input_cv_param_2 * cv_2_batch[batch_index];
				input_param_batch[batch_index] += cv_2_batch[batch_index];
			}

			if (inputs[CV1_INPUT].isConnected())
			{
				if (inputs[CV1_INPUT].getChannels() > 1)
				{
					cv_batch[batch_index] = xsimd::load_unaligned(&cv_voltage_ptr[x]);
				}
				else
				{
					cv_batch[batch_index] = cv_voltage_ptr[0];
				}
				input_cv_param = params[CV1_PARAM].getValue() * 0.1f;
				cv_batch[batch_index] = input_cv_param * cv_batch[batch_index];
				input_param_batch[batch_index] *= cv_batch[batch_index];
			}
