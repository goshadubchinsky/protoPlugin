...
struct PROTO6 : Module {
	...
	PROTO6() {
		...
		configParam(PARAM1_PARAM, 0.f, 10.f, 2.f, "drive up");
		configParam(PARAM2_PARAM, 0.f, 10.f, 2.f, "drive down");
	...
	{
		input_0 = inputs[IN1_INPUT].getVoltage();
		input_0 *= 0.2f;

		difference = input_0 - input_1;

		drive_1 = params[PARAM1_PARAM].getValue();
		drive_2 = params[PARAM2_PARAM].getValue();

		if (difference > 0.f)
		{output = std::tanh(M_PI / 2.f * input_0 * drive_1);}
		else if (difference < 0.f)
		{output = std::sin(M_PI / 2.f * clamp(drive_2 * input_0, -1.f, 1.f));}
		else
		{output = input_0;}

		input_1 = input_0;
		outputs[OUT1_OUTPUT].setVoltage(output*5.f);
	}
};
