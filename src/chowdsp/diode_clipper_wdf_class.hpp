...
public:
    ...
    void setSampleRate(float sampleRate)
    {
        ...
        C1.prepare((FloatType)this->sampleRate);
    }
    ...
    void setCircuitParams(FloatType input_gain, FloatType offset, FloatType cutoff)  //input_gain 0, 30, 1
    {
...
private:
    ...
    FloatType Cap{100.0e-9};
    FloatType applyInputGain(FloatType input)
    {
    ...
    FloatType sampleRate{44100.f};
    FloatType input_gain{1.f};
    FloatType offset{0.f};
    
    ResistorT<FloatType> R1 { 4700.f };
    ResistiveVoltageSourceT<FloatType> Vs;
    WDFSeriesT<FloatType, decltype (Vs), decltype (R1)> S1 { Vs, R1 };
    //CapacitorT<FloatType> C1 { 47.0e-9f }; // 47 nF - Ceramic
    CapacitorT<FloatType> C1 { 100.0e-9f }; // 100 nF - Ceramic
    //CapacitorT<FloatType> C1 { 10.0e-6f }; // 10 µF - Film
    //CapacitorT<FloatType> C1 { 22.0e-6f }; // 22 µF - Tantalum




    WDFParallelT<FloatType, decltype (S1), decltype (C1)> P1 { S1, C1 };
    //DiodePairT<FloatType, decltype (P1)> dp { P1, 2.52e-9f };           // GZ34         diode pair
    DiodePairT<FloatType, decltype (P1)> dp { P1, 4.0e-12 };           // 1N4148       diode pair
    //DiodePairT<FloatType, decltype (P1)> dp { P1, 15.0e-12f };          // 1N4007       diode pair
    //DiodePairT<FloatType, decltype (P1)> dp { P1, 2.0e-12f };           // BAT46        diode pair
    //DiodePairT<FloatType, decltype (P1)> dp { P1, 50.0e-12f };          // Zener        diode pair
