...
// constructor
SVFilter::SVFilter(double newCutoff,
                   double newResonance,
                   int newOversamplingFactor,
		               SVFFilterMode newFilterMode,
                   double newSampleRate,
		               SVFIntegrationMethod newIntegrationMethod,
                   int newDecimatorOrder)
{
  ...
  for(int nn = 0; nn < oversamplingFactor; nn++)
  {
    ...
    case SVF_TRAPEZOIDAL: ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
      // trapezoidal integration
      {
	      ...
	      double D_t = (1.0 - dt2*dt2/4.0)*bp + alpha*(u_t1 + input - 2.0*lp - fb*bp - SinhPade54(bp));
	      double x_k, x_k2;
    ...
    case SVF_INV_TRAPEZOIDAL:
      // inverse trapezoidal integration
      {
        ...
        double beta = 1.0 - (0.0075/oversamplingFactor);
        double alpha2 = dt2*dt2/4.0 + fb*alpha;
        double D_t = (1.0 - dt2*dt2/4.0)*bp + alpha*(u_t1 + input - 2.0*lp - fb*bp - sinh(bp));
        double y_k, y_k2;
        ...
        for(int ii=0; ii < SVF_MAX_NEWTON_STEPS; ii++)
        {
          y_k2 = y_k - (alpha*y_k + ASinhPade54(y_k) * (1.0 + alpha2) - D_t) / (alpha + (1.0 + alpha2) * dASinhPade54(y_k));
